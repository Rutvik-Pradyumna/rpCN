...
int main(){
    ...
    while(1){
        ...
        reply=msgrcv(msqidResponse,&responseMsgs,sizeof(responseMsgs),8,IPC_NOWAIT);
        if(reply!=-1) cout<<"1 : "<<responseMsgs.msgText<<endl;
        reply=msgrcv(msqidResponse,&responseMsgs,sizeof(responseMsgs),9,IPC_NOWAIT);
